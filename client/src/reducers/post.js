...
export default function (state = initialState, action) {
  ...
  switch (type) {
    ...
    case DELETE_POST:
      // State is immutable, therefore when adding another one it'll be an array. That's why we need to use spread operator to return all of them. On top of this, return "posts" (explained below in detail) as well as set "loading" to "false" once the request is done. It was removed from the server and in that way UI and state is also being updated.
      return {
        ...state,
    ...
    case UPDATE_LIKES:
      ...
      return {
        ...state,
        // Map through posts and check if that's the correct one to update (if ID's matches). If that's the correct one then return a new state with all what's in the post ("...post") and we want to manipulate just the likes to that posts which are returned. If it doesn't match the ID then simply return the post and don't do anything with it.
    ...
    case ADD_COMMENT:
      // State is immutable, therefore when adding another one it'll be an array. That's why we need to use spread operator to return all of them. On top of this, return "post" (explained below in detail) as well as set "loading" to "false" once the request is done.
      return {
        ...state,
        post: { ...state.post, comments: payload }, // Return all what's in the post ("...state.post") and we want to manipulate just the comments to that posts which are returned.
        loading: false,
      };
    case REMOVE_COMMENT:
      // State is immutable, therefore when adding another one it'll be an array. That's why we need to use spread operator to return all of them. On top of this, return "post" (explained below in detail) as well as set "loading" to "false" once the request is done.
      return {
        ...
        post: {
          ...
          comments: state.post.comments.filter(
            (comment) => comment._id !== payload // Return all comments except that one which matches here, because that's the one we want to delete. "payload" is in this case simply "id" of the comment as it was defined in post.js Actions ("deleteComment" method). It was removed from the server and in that way UI and state is also being updated.
          ),
        },
        loading: false,
      };
